        ...
        public void PlayerOnTick()
        {
            try
            {
                foreach (CCSPlayerController player in connectedPlayers.Values)
                {
                    ...
                    if (playerTimers.TryGetValue(playerSlot, out PlayerTimerInfo? playerTimer) && IsAllowedPlayer(player))
                    {
                        ...
                        string keysLineNoHtml = $"{(hudEnabled ? "<br>" : "")}<font class='fontSize-ml stratum-light-mono' color='{tertiaryHUDcolor}'>{((playerButtons & PlayerButtons.Moveleft) != 0 ? "🅐" : "_")} " +
                                                $"{((playerButtons & PlayerButtons.Forward) != 0 ? "🅦" : "_")} " +
                                                $"{((playerButtons & PlayerButtons.Moveright) != 0 ? "🅓" : "_")} " +
                                                $"{((playerButtons & PlayerButtons.Back) != 0 ? "🅢" : "_")} " +
                                                $"{((playerButtons & PlayerButtons.Jump) != 0 || playerTimer.MovementService!.OldJumpPressed ? "🅙" : "_")} " +
                        ...
                        if (enableReplays)
                        {
                            ...
                            {
                                if(timerTicks <= topReplayFrames && timerTicks <= maxReplayFrames)
            ...
            catch (Exception ex)
            {
                if (ex.Message != "Invalid game event") 
                {
                    ...
                    SharpTimerError($"Error in TimerOnTick: {ex.StackTrace}");
            	}
            }
        }
        ...
        public void SpectatorOnTick(CCSPlayerController player)
        {
            ...
            try
            {
                ...
                if (playerTimers.TryGetValue(target.Slot, out PlayerTimerInfo? playerTimer) && IsAllowedPlayer(target))
                {
                    ...
                    string keysLineNoHtml = $"{(hudEnabled ? "<br>" : "")}<font class='fontSize-ml stratum-bold-mono' color='{tertiaryHUDcolor}'>{((playerButtons & PlayerButtons.Moveleft) != 0 ? "🅐" : "_")} " +
                                            $"{((playerButtons & PlayerButtons.Forward) != 0 ? "🅦" : "_")} " +
                                            $"{((playerButtons & PlayerButtons.Moveright) != 0 ? "🅓" : "_")} " +
                                            $"{((playerButtons & PlayerButtons.Back) != 0 ? "🅢" : "_")} " +
                                            $"{((playerButtons & PlayerButtons.Jump) != 0 || playerTimer.MovementService!.OldJumpPressed ? "🅙" : "_")} " +
