        ...
        private void ReplayPlayback(CCSPlayerController player, int plackbackTick)
        {
            try
            {
                ...
                if (playerTimers.TryGetValue(player.Slot, out PlayerTimerInfo? value))
                {
                    ...
                    var replayButtons = $"{((replayFrame.Buttons & PlayerButtons.Moveleft) != 0 ? "🅐" : "_")} " +
                                        $"{((replayFrame.Buttons & PlayerButtons.Forward) != 0 ? "🅦" : "_")} " +
                                        $"{((replayFrame.Buttons & PlayerButtons.Moveright) != 0 ? "🅓" : "_")} " +
                                        $"{((replayFrame.Buttons & PlayerButtons.Back) != 0 ? "🅢" : "_")} " +
                                        $"{((replayFrame.Buttons & PlayerButtons.Jump) != 0 ? "🅙" : "_")} " +
                                        $"{((replayFrame.Buttons & PlayerButtons.Duck) != 0 ? "🅒" : "_")}";
        ...
        private async Task DumpReplayToJson(CCSPlayerController player, string steamID, int playerSlot, int bonusX = 0)
        {
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    if (playerReplays[playerSlot].replayFrames.Count > topReplayFrames || playerReplays[playerSlot].replayFrames.Count > maxReplayFrames - 64) return;
